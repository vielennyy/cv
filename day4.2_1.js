function next_birthday(horizont){
    ...
    // for (var j = current_month; j <= horizont; j++){
        ...
        for (var i = current_month; i <= horizont; i++){
            ...
            var m = employees_map.get(employees[i].month);
            for (var j = 0; j < m.length; j++)
            {
                var o = m[j];
                var age = current_year - o.year; 
                if (age % 10 == 1)
                    console.log(`(${o.date}) - ${o.person} (${age} год)`);
                else if (age % 10 == 2 || age % 10 == 3 || age % 10 == 4)
                    console.log(`(${o.date}) - ${o.person} (${age} года)`);
                else 
                    console.log(`(${o.date}) - ${o.person} (${age} лет)`);
            }
